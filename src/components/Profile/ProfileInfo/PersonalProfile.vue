    ...
    <fieldset>
      ...
      <div class="button-box" v-if="true">
        <v-slide-group>
          ...
          <template v-else>
            <v-btn class="ma-2" @click="closeForm" rounded> Close </v-btn>
            <v-btn class="ma-2" rounded type="submit"> Submit </v-btn>
          </template>
        </v-slide-group>
      </div>
    </fieldset>
...
const priorData = async () => {
  ...
  if (docSnap.exists()) {
    ...
    personal.fullName = personalData.fullName || registerData.fullName || "";
